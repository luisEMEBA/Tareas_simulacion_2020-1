...
public class choque_dos_dimensiones : MonoBehaviour
{
//EMPIEZA CREACIÓN DE VARIABLES DE INTERFAZ GRÁFICA
public Text ingreso_masa_obj_1, ingreso_masa_obj_2, ingreso_velocidad_obj1x,ingreso_velocidad_obj1y, ingreso_velocidad_obj2x,ingreso_velocidad_obj2y, ingreso_coeficiente_e, letrero;
string masa_1s, masa_2s, velocidad_inicial_x1s, velocidad_inicial_y1s, velocidad_inicial_x2s, velocidad_inicial_y2s, coeficiente_es;
//FINALIZA CREACIÓN DE VARIABLES DE INTERFAZ GRÁFICA
    ...
    float angulo, velocidad_obj1x, velocidad_obj2x, velocidad_obj1y, velocidad_obj2y, e;
    ...
    Debug.Log("El ángulo es : " + angulo); 
    }
public void iniciar(){
masa_1s = ingreso_masa_obj_1.text;
masa_2s = ingreso_masa_obj_2.text;
velocidad_inicial_x1s = ingreso_velocidad_obj1x.text;
velocidad_inicial_y1s = ingreso_velocidad_obj1y.text;
velocidad_inicial_x2s = ingreso_velocidad_obj2x.text;
velocidad_inicial_y2s = ingreso_velocidad_obj2y.text;
coeficiente_es = ingreso_coeficiente_e.text;
if(masa_1s == "" || masa_2s == "" || velocidad_inicial_x1s == "" || velocidad_inicial_x2s == "" || coeficiente_es == ""){
letrero.text = "Para empezar digite de manera correcta todos los valores iniciales.";
}
else{
float.TryParse(masa_1s, out masa1);
float.TryParse(masa_2s, out masa2);
float.TryParse(velocidad_inicial_x1s, out velocidad_obj1x);
float.TryParse(velocidad_inicial_x2s, out velocidad_obj2x);
float.TryParse(velocidad_inicial_y1s, out velocidad_obj1y);
float.TryParse(velocidad_inicial_y2s, out velocidad_obj2y);
float.TryParse(coeficiente_es, out e);
control = 1;    
}
}
public void reiniciar(){
ingreso_velocidad_obj2x.text = "";
ingreso_velocidad_obj1x.text = "";
ingreso_velocidad_obj2y.text = "";
ingreso_velocidad_obj1y.text = "";
...
velocidad_obj2x = 0;
velocidad_obj2y = 0;
masa_1.gameObject.GetComponent<Transform>().position = new Vector3(-3,1,0);
masa_2.gameObject.GetComponent<Transform>().position = new Vector3(3,1,0);
Objetivo1.gameObject.GetComponent<Transform>().position = new Vector3(4,1,0);
Objetivo2.gameObject.GetComponent<Transform>().position = new Vector3(-4,1,0);
control = 0;
control_1 = 0;
control_2 = 0;
control_angulo = 0;
